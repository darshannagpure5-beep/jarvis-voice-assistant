...
.orb-container { display: flex; justify-content: center; padding: 1.5rem 0; }
.orb { width: 120px; height: 120px; border-radius: 50%; background: radial-gradient(circle at 35% 35%, #00d4ff55, #0a1628 70%); border: 1.5px solid #00d4ff44; box-shadow: 0 0 30px #00d4ff22; transition: all 0.3s ease; }
.orb.thinking { box-shadow: 0 0 40px #ff6b0066; animation: spin-glow 1.5s linear infinite; border-color: #ff6b0099; }
.orb.speaking { box-shadow: 0 0 40px #00ff8866; animation: pulse 0.8s ease-in-out infinite; border-color: #00ff8899; }
...
.status-dot.thinking { background: #ff6b00; }
.status-dot.speaking { background: #00ff88; }
@keyframes blink { 0%, 100% { opacity: 1; } 50% { opacity: 0.3; } }
.chat-container { max-height: 380px; overflow-y: auto; padding: 0.5rem 0; }
...
.stTextInput > div > div > input { background: #0a1628 !important; border: 1px solid #1a3a5c !important; border-radius: 8px !important; color: #a8d8ff !important; font-size: 1rem !important; }
.divider { border: none; border-top: 1px solid #0d2840; margin: 1rem 0; }
.reminder-pill { display: inline-block; background: #0a1f2e; border: 1px solid #00d4ff22; border-radius: 20px; padding: 3px 12px; font-size: 0.7rem; color: #3a9bd5; margin: 2px; }
</style>
...
st.markdown(\"<div class=jarvis-header><p class=jarvis-title>JARVIS</p><p class=jarvis-sub>JUST A RATHER VERY INTELLIGENT SYSTEM</p></div>\", unsafe_allow_html=True)
...
st.markdown(f\"<div class=orb-container><div class=\\\"orb {orb_class}\\\"></div></div><div class=status-badge><span class=\\\"status-dot {dot_class}\\\"></span>{status_text}</div>\", unsafe_allow_html=True)

st.markdown(\"<div class=chat-container>\", unsafe_allow_html=True)
for msg in st.session_state.messages:
    role = msg[\"role\"]
    label = \"YOU\" if role == \"user\" else \"JARVIS\"
    st.markdown(f\"<div class=\\\"msg-row {role}\\\"><div class=\\\"bubble {role}\\\"><div class=bubble-label>{label}</div>{msg[chr(39)content chr(39)]}</div></div>\", unsafe_allow_html=True)
...
if st.session_state.reminders:
    ...
    for r in st.session_state.reminders:
        st.markdown(f\"<span class=reminder-pill>Reminder: {r}</span>\", unsafe_allow_html=True)
'''
